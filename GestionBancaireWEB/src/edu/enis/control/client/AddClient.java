...
public class AddClient extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    ...
    try {
      ...
      if (cin.trim().length() != 8) throw new InvalidNameException(cin);
      if (firstName.trim().length() < 3) throw new InvalidNameException(firstName);
      if (lastName.trim().length() < 3) throw new InvalidNameException(lastName);
      if (address.trim().length() < 3) throw new InvalidNameException(address);
    ...
    if (request.getAttribute("error") != null) {
      ...
      request.getRequestDispatcher("/ListAllAccounts").forward(request, response);
    }
